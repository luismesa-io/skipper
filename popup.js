...
if (chrome.storage) {
    chrome.storage.local.get("active", async ({ active }) => {
        if (active === true) {
            ...
            document.getElementById('toggleLabel').innerHTML = 'Active';
            ...
            document.getElementById('toggleLabel').innerHTML = 'Inactive';
        }
    });
}
...
function toggleObservation() {
    ...
    chrome.storage.local.get("active", ({ active }) => {
        if (active) {
            ...
            document.mutationObserver.observe(document.getElementById('ytd-player'), { attributes: false, childList: true, subtree: true });
        }
    });
}

toggleButton.addEventListener('click', async () => {
    chrome.storage.local.set({ "active": toggleButton.checked }, function () {
